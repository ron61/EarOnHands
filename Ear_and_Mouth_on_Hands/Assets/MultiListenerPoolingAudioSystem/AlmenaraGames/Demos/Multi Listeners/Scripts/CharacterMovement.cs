...
public class CharacterMovement : MonoBehaviour {
	...
	private MultiAudioListener listener;
	...
	private bool insideReverbZone = false;
	...
	//Get Multi Audio Listener
	listener = GetComponent<MultiAudioListener>();
	...
	StartCoroutine (CheckForReverbTrigger ());
	...
	void Update () {
		...
		//Check if the player is inside a Reverb Zone
		IEnumerator CheckForReverbTrigger()
		{
			while (true) {
				bool inReverbZone = false;
				Collider[] players = Physics.OverlapSphere (transform.position, 1f, -1, QueryTriggerInteraction.Collide);

				foreach (var item in players) {

					if (item.GetComponent<ReverbTrigger>()!=null) {
						inReverbZone = true;
					}

				}

				insideReverbZone = inReverbZone;
